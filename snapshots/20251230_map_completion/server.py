...
class Message(db.Model):
    ...
    coord_x = db.Column(db.Float, nullable=False)  # 경도 (longitude)
    coord_y = db.Column(db.Float, nullable=False)  # 위도 (latitude)
    tags = db.Column(db.String(50))  # 해시태그 (예: #발길 #맛집)
    address = db.Column(db.String(300))  # 전체 주소
    address_base = db.Column(db.String(200))  # 기본 주소 (상세 주소 제외)
...
class Route(db.Model):
    """이동 경로 기록 (집단지성용, 개인정보는 클라이언트에 저장)"""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.String(100), nullable=False)

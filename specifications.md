...
## 1. 개요
- **목표**: 배달원과 휠체어 이용자를 위한 라스트 마일 내비게이션 및 소셜 매핑 플랫폼
- **핵심 가치**: 초정밀 경로 안내, 보행 약자 친화적 정보, 커뮤니티 기반 데이터 구축
...
### 백엔드
- **기술 스택**: Python Flask
- **데이터베이스**: SQLAlchemy ORM (개발: SQLite / 배포: PostgreSQL)
...
- **이동 모드 특성**:
  - 도보(Walking): 일반적인 최단 거리 도보 경로
  - 휠체어(Wheelchair): 경사로, 엘리베이터 등 배리어 프리 경로 우선 및 시뮬레이션
- **혼잡도 시각화 (Trajectory)**:
  - 1~3명: 한적 (여유로움)
  - 4~10명: 보통
  - 11~100명: 활발
